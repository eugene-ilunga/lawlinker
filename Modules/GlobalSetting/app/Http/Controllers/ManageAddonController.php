...
class ManageAddonController extends Controller
{
    ...
    public function installProcessStart()
    {
        ...
        if (pathinfo($file, PATHINFO_EXTENSION) === 'zip' && $this->isFirstDirAddons($file)) {
            ...
            if (property_exists($addonFile, 'minimum_version') && !empty($addonFile?->minimum_version)) {
            ...
            if (isset($addonFileJson['item'])) {
                ...
                if (!(isset($item['project_name']) && $item['project_name'] == 'LawMent' && isset($item['unique_id']) && Hash::check('33132125', $item['unique_id']) )) {
                    return redirect()->back()->with([
