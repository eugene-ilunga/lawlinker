...
class PaymentMethodService implements PaymentMethodInterface {
     ...
     *
     */
    protected static array $additionalActiveApiGateways = [];
    /**
     * Add additional active gateways to the list.
     *
     * @param array $additionalActiveApiGatewaysList
     */
    public static function additionalApiActiveGatewaysList(array $additionalActiveApiGatewaysList): void {
        static::$additionalActiveApiGateways = array_merge(static::$additionalActiveApiGateways, $additionalActiveApiGatewaysList);
    }
    public function getActiveApiGatewaysWithDetails(): array {
        $basicPayment = $this->get_basic_payment_info();
        $activeStatus = config('basicpayment.default_status.active_text');
        ...
        // Merge base gateways with additional gateways
        $allGateways = array_merge($gateways, static::$additionalActiveApiGateways);

        // Filter only active gateways
        return array_filter($allGateways, fn($gateway) => $gateway['status'] == 'active');
    }
}
